...
def step_impl(
    ...
    ready_up_loads = [
        DynStreamify(
            ...
            expert_feature_streams[i],  # [Dyn // tile_N,]
            0,
            1,
        )
        for i in range(E)
    ...
    down_feature_streams = [
        RetileStreamify(
            step_graph,
            chunked_down_feature_streams[i],  # [Dyn // tile_N]
            True
        )
        for i in range(E)
    ] # [Dyn]
    ...
    # [Genghan] How to deal with padded shapes in reshape?
    # Otherwise, Input streams must have the same shape: (4*ceiling(SelectGen_26_0/4),) != (SelectGen_26_0,)
    for i in range(E):
        expert_weight_streams._stream[i].shape = unchunked_expert_feature_streams._stream[i].shape
        down_feature_streams[i]._stream.shape = unchunked_expert_feature_streams._stream[i].shape
