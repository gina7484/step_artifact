...
def subsFullShape(dtype, shape, symbols):
    if isinstance(dtype, Buffer):
        if isinstance(dtype.dtype, Multihot) or isinstance(dtype.dtype, Vector) or isinstance(dtype.dtype, Tile):
            shape = dtype.shape + shape
        else:
            raise ValueError("Buffer with non-vector, non-tile or non-multihot dtype is not supported.")
    elif isinstance(dtype, Vector) or isinstance(dtype, Tile) or isinstance(dtype, Multihot):
        shape = shape
    else:
...
def subsFlattenFullShape(dtype, shape, symbols):
    if isinstance(dtype, Buffer):
        if isinstance(dtype.dtype, Multihot):
            shape = [dtype.dtype.length] + dtype.shape + shape
        elif isinstance(dtype.dtype, Vector) or isinstance(dtype.dtype, Tile):
            shape = dtype.dtype.block + dtype.shape + shape
        else: 
            raise ValueError("Buffer with non-scalar or non-multihot dtype is not supported.")
    elif isinstance(dtype, Vector) or isinstance(dtype, Tile):
        shape = dtype.block + shape
...
class Vector(Element):
    def __init__(self, dtype, block: list[Symbol]):
        super().__init__(dtype)
        self.block = block
        self.rank = len(block)
        assert self.rank == 1, "Vector must have rank 1"
    
    def __str__(self):
        return f"Tile: {self.dtype}, [{', '.join([str(s) for s in self.block])}]"
    
    def __eq__(self, other):
        if not isinstance(other, Vector):
            return False
        return self.dtype == other.dtype and self.block == other.block
...
class Tile(Element):
    def __init__(self, dtype, block: list[Symbol]):
        ...
        self.block = block
        self.rank = len(block)
        assert self.rank == 2, "Tile must have rank 2"
