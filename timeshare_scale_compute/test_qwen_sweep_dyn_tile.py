...
from timeshare_scale_compute.test_weight_stationary_gemm_dyn_tile import (
...
def run_timeshare_mn_mk_gemm_reshape_fixed_fanout_dyn_tile(
    ...
    GATE_COMPUTE_BW = flops * expert_per_region
    UP_COMPUTE_BW = flops * expert_per_region
    ACT_FN_COMPUTE_BW = flops * expert_per_region
    MULT_COMPUTE_BW = flops * expert_per_region
    DOWN_COMPUTE_BW = flops * expert_per_region
...
def test_timeshare_mn_mk_gemm_reshape_dyn_tile():
    ...
    out_file = (
        f"timeshare_scale_compute/"
        + f"qwen_{model_config.dim}_{model_config.moe_inter_dim}_"
        + f"iter{iter:03d}_layer_{layer:03d}_round_{round_N}_f{tile_F}_"
        + f"timeshare_scale_compute_dyn_tile_"
...
def test_baseline_for_scale_compute_timeshare():
    mock_bf16 = True
    par_dispatch = 1
    # ------------ Model Configuration ------------
    # model_config = SmallerQwen30b()
    ...
    out_file = (
        ...
        + f"qwen_{model_config.dim}_{model_config.moe_inter_dim}_"
        + f"iter{iter:03d}_layer_{layer:03d}_round_{round_N}_f{tile_F}_"
        + f"timeshare_scale_compute_baseline_dyn_tile_"
        + f"{time.strftime("%d%H%M%S")}.csv"
    )
    try:
        with open(out_file, "w", newline="", encoding="utf-8") as csvfile:
