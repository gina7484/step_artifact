...
def test_prefill_expert_mnk_mnk():
    ...
    in_load = OffChipLoad(
        underlying=input,  # [B, N, H] => [B, N // gate_up_tile_config.m, H // gate_up_tile_config.k]
        stride=(
        ...
        par_dispatch=4,
    )  # [1, B, N // gate_up_tile_config.m, H // gate_up_tile_config.k]

    buff_input = Bufferize(
        graph=step_graph,
        input=in_load,
        rank=1,
    )  # [1, B, N // gate_up_tile_config.m]
    ...
    formatted_input = Streamify(
        ...
        rank=1,
    )  # [1, B, N // gate_up_tile_config.m, MLP_HID // gate_up_tile_config.n, H // gate_up_tile_config.k]
    ...
    #     stride=(1, H // gate_up_tile_config.k),
    #     out_shape_tiled=(
    #         MLP_HID // gate_up_tile_config.n,
    #         H // gate_up_tile_config.k,
    #     ),
    ...
    print = OffChipStore(graph=step_graph, input=gate, par_dispatch=4)
    # Linear (Up)
    # up = BinaryMapAccum(
    #     graph=step_graph,
    #     in1=formatted_input,
    ...
    # Linear (Down)
    # bufferize 1D
    # streamify (H // down_tile_config.n)
    # BinaryMapAccum (1d)

    # Output
    # output_gate = OffChipStore(
    #     graph=step_graph,
    #     input=gate,
    #     par_dispatch=4,
    #     store_file_name="gate_output",
    # )
    # output_up = OffChipStore(
    #     graph=step_graph,
    #     input=up,
    #     par_dispatch=4,
    #     store_file_name="up_output",
    # )
