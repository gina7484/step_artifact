...
def test_gemm_sweep():
    ...
    for tile_N in tile_Ns:
        for tile_F in tile_Fs:
            ...
            # out_file = f"qwen_{model_config.dim}_{model_config.moe_inter_dim}_iter{iter:03d}_layer_{layer:03d}_n{tile_N}_f{tile_F}.csv"
            # try:
...
def test_gemm_dyn_tile():
    ...
    for tile_F in tile_Fs:
        ...
        out_file = f"qwen_{model_config.dim}_{model_config.moe_inter_dim}_round_{round_N}_iter{iter:03d}_layer_{layer:03d}_n_dyn_f{tile_F}.csv"
        try:
            with open(out_file, "w", newline="", encoding="utf-8") as csvfile:
                fieldnames = [
                    "batch",
